...
int main(int argc, char* argv[]) {
  ...
  testSettings.min_duration_ms =
    60000;  // TO-DO: expose it to the outer level user command
  ...
  amdinfer::QuerySampleLibrary qsl(10, "/tmp/mlperf/", pre_process_resnet50);

  amdinfer::ParameterMap parameters;
  parameters.put("model",
                 "/workspace/amdinfer/external/artifacts/u200_u250/"
                 "resnet_v1_50_tf/resnet_v1_50_tf.xmodel");
  parameters.put("batch_size", 4);
  amdinfer::SystemUnderTestNative sut(&qsl, "xmodel", parameters);
