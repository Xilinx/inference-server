...
RocalOperation getOperationType(const std::string& op) {
  ...
  if (op == "ResizeMirrorNormalize")
    return RocalOperation::ResizeMirrorNormalize;
  ...
  if (op == "ResizeCropMirrorFixed")
    return RocalOperation::ResizeCropMirrorFixed;
  std::cerr << "Unknown operation: " + op << std::endl;
  throw invalid_argument("Unknown operation: " + op);
}
/**
 * @brief A helper function to convert RocalTensorOutputType to
 * amdinfer::DataType
 */
...
void RocalWorker::deserialize(const std::string& model_path) {
  ...
  if (doc.HasParseError()) {
    std::cerr << "JSON parsing error: " + std::to_string(doc.GetParseError()) +
                   " at offset " + std::to_string(doc.GetErrorOffset())
              << std::endl;
    throw file_read_error(
      "JSON parsing error: " + std::to_string(doc.GetParseError()) +
      " at offset " + std::to_string(doc.GetErrorOffset()));
  } else {
    std::cout << "rocAL Model Parsing Complete" << std::endl;
  }

  [[maybe_unused]] RocalTensor input = rocalJpegExternalFileSource(
    handle_, color_format_, false, false, false, ROCAL_USE_USER_GIVEN_SIZE,
    max_decode_width_, max_decode_height_,
    RocalDecoderType::ROCAL_DECODER_TJPEG, rocal_external_mode_);
  ...
  for (const auto& node : pipeline.GetArray()) {
    RocalOperation operationType =
      getOperationType(node["operation"].GetString());
    ...
    switch (operationType) {
      case RocalOperation::Resize:
        ...
        is_output =
          node.HasMember("is_output") ? node["is_output"].GetBool() : false;
        outputTensor =
          rocalResize(handle_, inputTensor, width, height, is_output);
        break;
      case RocalOperation::ResizeMirrorNormalize:
        ...
        outputTensor = rocalResizeMirrorNormalize(
          handle_, inputTensor, width, height, mean, std_dev, is_output,
          ROCAL_SCALING_MODE_DEFAULT, {}, 0, 0, ROCAL_LINEAR_INTERPOLATION,
          p_mirror);
        break;
      case RocalOperation::CropResizeFixed:
        ...
        outputTensor = rocalCropResizeFixed(handle_, inputTensor, width, height,
                                            is_output, area, aspect_ratio,
                                            x_center_drift, y_center_drift);
        break;
      case RocalOperation::CropMirrorNormalize:
        ...
        p_mirror = rocalCreateIntParameter(mirror);
        outputTensor =
          rocalCropMirrorNormalize(handle_, inputTensor, height, width, x, y,
                                   mean, std_dev, is_output, p_mirror);
        break;
      case RocalOperation::CropFixed:
        ...
        outputTensor = rocalCropFixed(handle_, inputTensor, width, height,
                                      depth, is_output, x, y, z);
        break;
      case RocalOperation::ResizeCropMirrorFixed:
        ...
        mirror = node.HasMember("mirror") ? node["mirror"].GetBool() : false;
        p_mirror = rocalCreateIntParameter(mirror);
        outputTensor = rocalResizeCropMirrorFixed(
          handle_, inputTensor, width, height, is_output, y, x, p_mirror);
        break;
      default:
...
void RocalWorker::doInit(ParameterMap* parameters) {
  ...
  this->handle_ =
    rocalCreate(batch_size_, RocalProcessMode::ROCAL_PROCESS_CPU, 0, 1);
  if (rocalGetStatus(this->handle_) != ROCAL_OK) {
    AMDINFER_LOG_ERROR(logger, "RocalWorker handle creation failed");
    // Throwing an exception causes server to delete this worker instance.
    // Client must try again.
    throw runtime_error("could not create rocAL context");
  }
...
BatchPtr RocalWorker::doRun(Batch* batch,
  ...
  for (unsigned int j = 0; j < batch->size(); j++) {
    ...
    assert(input_shape.size() ==
           1);  // compressed raw buffer should have shape length = 1
  ...
  bool eos =
    true;  // eos value is always true since there will be only one input tensor
  rocalExternalSourceFeedInput(
    handle_, {}, false, input_batch_buffer, ROI_xywh_, max_decode_width_,
    max_decode_height_, channels_,
    RocalExternalSourceMode(rocal_external_mode_), RocalTensorLayout(0), eos);
  ...
  for (unsigned int j = 0; j < batch->size(); j++) {
    ...
    new_request->addInputTensor(
      nullptr, shape, toDataType(output_tensor->data_type()), "output");
    new_batch->addRequest(new_request);
  }

  std::vector<amdinfer::BufferPtr> input_buffers;
  input_buffers.emplace_back(
    std::make_unique<amdinfer::VectorBuffer>(max_decoded_size * batch_size_));
  ...
  for (unsigned int k = 0; k < batch->size(); k++) {
    ...
    amdinfer::util::copy(data, static_cast<std::byte*>(data_ptr), decoded_size);
  ...
  auto duration =
    timer.count<std::micro>("rocal_batch_start", "rocal_batch_end");
  AMDINFER_LOG_INFO(logger, "rocAL preprocessing completed in " +
