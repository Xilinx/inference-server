...
int ClassificationPostProc::exec_async(std::vector<vart::TensorBuffer *> &in,
                                       std::vector<vart::TensorBuffer *> &out,
                                       AKS::NodeParams *nodeParams,
                                       AKS::DynamicParamValues *dynParams) {
  ...
  int inSize = std::accumulate(std::next(inShape.begin()), inShape.end(), 1,
                               std::multiplies<int>());
  ...
  int k = nodeParams->_intParams.find("top_k") == nodeParams->_intParams.end()
            ? 5
            : nodeParams->_intParams["top_k"];

  std::string tensor_name_labels("post-labels");
  std::string tensor_name_probs("post-probs");

  AKS::AksTensorBuffer *labels = new AKS::AksTensorBuffer(xir::Tensor::create(
    tensor_name_labels, {inBatch, k, 1, 1}, xir::create_data_type<int>()));

  AKS::AksTensorBuffer *probs = new AKS::AksTensorBuffer(xir::Tensor::create(
    tensor_name_probs, {inBatch, k, 1, 1}, xir::create_data_type<float>()));
