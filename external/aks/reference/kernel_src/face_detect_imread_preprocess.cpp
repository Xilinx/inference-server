...
int FaceDetectImreadPre::exec_async(std::vector<vart::TensorBuffer*>& in,
                                    std::vector<vart::TensorBuffer*>& out,
                                    AKS::NodeParams* nodeParams,
                                    AKS::DynamicParamValues* dynParams) {
  ...
  std::vector<float> mean{meanIter->second[0], meanIter->second[1],
                          meanIter->second[2]};
  ...
  AKS::AksTensorBuffer* outTB = new AKS::AksTensorBuffer(
    xir::Tensor::create("face-preproc", shape, xir::create_data_type<float>()));
