...
void FaceDetectPostProc::nodeInit(AKS::NodeParams* nodeParams) { _stride = 8; }

int FaceDetectPostProc::exec_async(std::vector<vart::TensorBuffer*>& in,
                                   std::vector<vart::TensorBuffer*>& out,
                                   AKS::NodeParams* nodeParams,
                                   AKS::DynamicParamValues* dynParams) {
  ...
  if (tensor0_channel == 2 && tensor1_channel == 4) {
    pc_idx = 0;
    bb_idx = 1;
  } else if (tensor0_channel == 4 && tensor1_channel == 2) {
    pc_idx = 1;
    bb_idx = 0;
  } else {
    std::cout << "Input Tensors channel error: "
              << "Tensor 0 channel is " << tensor0_channel << " "
              << "Tensor 1 channel is " << tensor1_channel << " " << std::endl;
  }
  ...
  auto bb_in_size = std::accumulate(std::next(bbShape.begin()), bbShape.end(),
  ...
  auto pc_in_size = std::accumulate(std::next(pcShape.begin()), pcShape.end(),
                                    1, std::multiplies<int>());
  ...
  for (int b = 0; b < input_batch; ++b) {
    ...
    if (do_gstiling) {
      ...
      GSTilingLayer(bb_out, input_batch, bb_in_channel, bb_in_height,
                    bb_in_width, bb_out_channel, stride, bb_out_height,
                    bb_out_width, gst_tiling_bb.get());
      //# perform GSTiling on pixel_conv
      GSTilingLayer(pc_out, input_batch, pc_in_channel, pc_in_height,
                    pc_in_width, pc_out_channel, stride, pc_out_height,
                    pc_out_width, gst_tiling_pc.get());
    ...
    /// Perform SoftMax
    softmax(pc_out, pc_scale, pc_out_channel, pc_tiling_size / pc_out_channel,
            conf.data());

    /// Filter Boxes
    vector<vector<float>> boxes =
      FilterBox(bb_scale, det_threshold, bb_out, bb_out_width, bb_out_height,
                conf.data());
  ...
  AKS::AksTensorBuffer* outTB = new AKS::AksTensorBuffer(xir::Tensor::create(
    "face-detect-out", {out_tensor_size}, xir::create_data_type<float>()));
