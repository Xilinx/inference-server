...
void getResult(const vector<vector<float>>& box, const vector<int>& keep,
...
void NMS(const float nms_threshold, const vector<vector<float>>& box,
         vector<vector<float>>& results) {
  auto count = box.size();
...
vector<vector<float>> FilterBox(const float bb_out_scale,
                                const float det_threshold, float* bbout, int w,
                                int h, float* pred) {
...
void softmax_c(float* input, float scale, unsigned int cls, float* output) {
  float sum = 0.f;
  float max = input[0];
...
void softmax_c(float* input, float scale, unsigned int cls, unsigned int group,
               float* output) {
...
void softmax(float* input, float scale, unsigned int cls, unsigned int group,
             float* output) {
  softmax_c(input, scale, cls, group, output);
}

void GSTilingLayer(float* in_data, int input_batch, int input_channels,
                   int input_height, int input_width, int output_channels,
                   int stride, int output_height, int output_width,
                   float* out) {
  int ox, oy, oc, oi;
  int n, ic, iy, ix;
