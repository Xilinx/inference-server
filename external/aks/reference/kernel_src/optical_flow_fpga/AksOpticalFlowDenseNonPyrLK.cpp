...
void OpticalFlowDenseNonPyrLK::nodeInit(AKS::NodeParams *params) {
  ...
  // lk_kName = params->_stringParams.find("lk_kName") ==
  // params->_stringParams.end() ?
  //   "dense_non_pyr_of_accel" : params->_stringParams["lk_kName"];
  // of_width = params->_intParams["of_w"];
  // of_height = params->_intParams["of_h"];

  lk_kName =
    params->_stringParams.find("lk_kName") == params->_stringParams.end()
      ? "dense_non_pyr_of_accel"
      : params->getValue<std::string>("lk_kName");
  ...
  cl_int err;
  cl_int status;               // error code returned from api calls
  ...
  command_queue =
    clCreateCommandQueue(context, device_id, commandQueueProperties, &err);
  if (!command_queue) {
...
int OpticalFlowDenseNonPyrLK::getNumCUs(void) { return 1; }

int OpticalFlowDenseNonPyrLK::exec_async(vector<vart::TensorBuffer *> &in,
                                         vector<vart::TensorBuffer *> &out,
  ...
  err |= clSetKernelArg(of_kernel, 0, sizeof(ocl_of_in_prev_buf),
                        &ocl_of_in_prev_buf);
  err |=
    clSetKernelArg(of_kernel, 1, sizeof(ocl_of_in_cur_buf), &ocl_of_in_cur_buf);
  ...
  err = clEnqueueWriteBuffer(command_queue, ocl_of_in_prev_buf, CL_TRUE, 0,
                             (of_width * of_height) * sizeof(unsigned char),
                             prevData, 0, NULL, NULL);
  if (err != CL_SUCCESS) {
    printf("Error: Failed to write to source array error = %d!\n", err);
  }
  err = clEnqueueWriteBuffer(command_queue, ocl_of_in_cur_buf, CL_TRUE, 0,
                             (of_width * of_height) * sizeof(unsigned char),
                             currData, 0, NULL, NULL);
  if (err != CL_SUCCESS) {
  ...
  err = clEnqueueReadBuffer(command_queue, ocl_flowx, CL_TRUE, 0,
                            (of_height * of_width) * sizeof(float),
                            flowx.data(), 0, NULL, &readevent);
  if (err != CL_SUCCESS) {
  ...
  if (err != CL_SUCCESS) {
    printf(
      "Error: Failed to clEnqueueReadBuffer flowx clWaitForEvents error %d!\n",
      err);
  }

  err = clEnqueueReadBuffer(command_queue, ocl_flowy, CL_TRUE, 0,
                            (of_height * of_width) * sizeof(float),
                            flowy.data(), 0, NULL, &readevent1);
  if (err != CL_SUCCESS) {
  ...
  if (err != CL_SUCCESS) {
    printf(
      "Error: Failed to clEnqueueReadBuffer flowy clWaitForEvents error %d!\n",
      err);
  }
