...
void ClassificationAccuracy::setAccuracyData(AKS::NodeParams* nodeParams,
                                             AccuracyData* accuData) {
  _accuracyDatas[nodeParams] = accuData;
}

AccuracyData* ClassificationAccuracy::getAccuracyData(
  AKS::NodeParams* nodeParams) {
...
void ClassificationAccuracy::nodeInit(AKS::NodeParams* nodeParams) {
  ...
  if (!accuData) {
    ...
    while (getline(gtFile, line)) {
      ...
      accuData->_groundTruth[imgNameAndIndex[0]] =
        std::stoi(imgNameAndIndex[1]);
...
void ClassificationAccuracy::updateTopK(AKS::NodeParams* nodeParams,
                                        std::string& path, int* topK) {
  AccuracyData* accuData = getAccuracyData(nodeParams);
  if (accuData) {
...
int ClassificationAccuracy::exec_async(std::vector<vart::TensorBuffer*>& in,
                                       std::vector<vart::TensorBuffer*>& out,
                                       AKS::NodeParams* nodeParams,
                                       AKS::DynamicParamValues* dynParams) {
  // Start the timer
...
void ClassificationAccuracy::report(AKS::NodeParams* nodeParams) {
  ...
  AccuracyData* accuData = getAccuracyData(nodeParams);
  if (accuData) {
    _INFO << "Accuracy: Total Images Processed: " << accuData->_imagesProcessed
          << std::endl;
    _INFO << "Accuracy: Top 1: "
          << 100 * (float(accuData->_top1) / float(accuData->_imagesProcessed))
          << std::endl;
    _INFO << "Accuracy: Top 5: "
          << 100 * (float(accuData->_top5) / float(accuData->_imagesProcessed))
          << std::endl;
  }

  auto dur = std::chrono::duration<float>{_t1 - _t0}.count();
  LOG_X(DEBUG) << "Total time (s) around Accuracy Layer : " << dur << std::endl;
  LOG_X(DEBUG) << "FPS around Accuracy Layer : "
               << accuData->_imagesProcessed / dur << std::endl;
}
