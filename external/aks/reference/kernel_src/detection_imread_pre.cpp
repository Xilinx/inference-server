  ...
  if (dx == 0) {
    ...
    for (int rows = 0; rows < source.rows; ++rows) {
      ...
      unsigned long dstOffset =
        rows * dest.cols * dest.channels() + (dx * dest.channels());
      memcpy(dstData + dstOffset, srcData + srcOffset, moveBytes);
    }
  }
}

void letterBoxImage(cv::Mat& inImage, int resizeH, int resizeW,
                    cv::Mat& outImage) {
  ...
  embedImage(scaledImage, outImage, (resizeW - new_w) / 2,
             (resizeH - new_h) / 2);
}

int DetectionImreadPre::exec_async(std::vector<vart::TensorBuffer*>& in,
                                   std::vector<vart::TensorBuffer*>& out,
                                   AKS::NodeParams* nodeParams,
                                   AKS::DynamicParamValues* dynParams) {
  ...
  imgDims.reserve(3 * batchSize);
  for (int i = 0; i < batchSize; ++i) {
    cv::Mat inImage = cv::imread(dynParams->imagePaths[i]);
    if (!inImage.data) {
      std::cerr << "[ERR] Unable to read image: " << dynParams->imagePaths[0]
                << std::endl;
      return -2;
    }
