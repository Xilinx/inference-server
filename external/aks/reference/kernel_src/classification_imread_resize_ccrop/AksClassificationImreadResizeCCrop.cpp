...
class ClassificationImreadResizeCCrop : public AKS::KernelBase {
 public:
...
AKS::KernelBase* getKernel(AKS::NodeParams* params) {
  return new ClassificationImreadResizeCCrop();
}

}  // extern "C"

int ClassificationImreadResizeCCrop::exec_async(
  ...
  std::string tensorName("pre-float-out");

  AKS::AksTensorBuffer* outDD = new AKS::AksTensorBuffer(
    xir::Tensor::create(tensorName, shape, xir::create_data_type<float>()));
  ...
  for (int i = 0; i < batchSize; ++i) {
    ...
    if (!inImage.data) {
      std::cerr << "[ERR] Unable to read image: " << dynParams->imagePaths[i]
                << std::endl;
      return -2;
    }

    cv::cvtColor(inImage, inImage, cv::COLOR_BGR2RGB);
    /// Reize and Crop
    int new_dim = std::min(inImage.cols, inImage.rows);

    float scaleH = float(inImage.rows) / new_dim;
    float scaleW = float(inImage.cols) / new_dim;

    int min_dim = std::min(outHeight, outWidth);

    int newHeight = min_dim * scaleH;
    int newWidth = min_dim * scaleW;

    cv::Mat resizedImage = cv::Mat(cv::Size(newWidth, newHeight), CV_8UC3);
    cv::resize(inImage, resizedImage, cv::Size(newWidth, newHeight));

    int offsetW = (newWidth - outWidth) / 2;
    int offsetH = (newHeight - outHeight) / 2;

    cv::Rect roi(offsetW, offsetH, outWidth, outHeight);
