...
class DPURunner : public AKS::KernelBase {
 public:
  ...
  int exec_async(std::vector<vart::TensorBuffer*>& in,
                 std::vector<vart::TensorBuffer*>& out, AKS::NodeParams* params,
...
void DPURunner::nodeInit(AKS::NodeParams* params) {
  nodes.emplace(std::piecewise_construct, std::forward_as_tuple(params),
  ...
  auto num_runners = params->hasKey<int>("num_runners")
                       ? params->getValue<int>("num_runners")
                       : 1;
  // Load graph
  nodes[params].graph = std::move(xir::Graph::deserialize(xmodelFile));
  // Get DPU subgraph and create runners
  nodes[params].subgraphs =
    std::move(get_dpu_subgraph(nodes[params].graph.get()));
  for (int i = 0; i < num_runners; ++i) {
    std::unique_ptr<vart::Runner> runner_ =
      vart::Runner::create_runner(nodes[params].subgraphs.back(), "run");
    nodes[params].runners.push_back(std::move(runner_));
  }
  nodes[params].core_count = num_runners;
}

int DPURunner::exec_async(vector<vart::TensorBuffer*>& in,
  ...
  for (auto& tensor : inputTensors) {
    ...
    assert((in_shape == tensor->get_shape()) &&
           "[ERROR] Input shape does not match DPU input shape!");
    if (tensor->get_name() != in[index]->get_tensor()->get_name()) {
      xir::Tensor* inputTensor =
        const_cast<xir::Tensor*>(in[index]->get_tensor());
  ...
  for (auto& tensor : outputTensors) {
    // Create o/p buffers
    AKS::AksTensorBuffer* outDD = new AKS::AksTensorBuffer(xir::Tensor::create(
      tensor->get_name(), tensor->get_shape(), xir::create_data_type<float>()));
    out.push_back(outDD);
